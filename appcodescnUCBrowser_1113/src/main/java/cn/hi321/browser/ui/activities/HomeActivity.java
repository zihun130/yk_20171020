...
public class HomeActivity extends Activity implements OnTouchListener,
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (i.getData() != null) {
			...
			addTab(false);
			navigateToUrl(i.getDataString());// i.getDataString()获得�?��装的包名
		} else {
	...
	private void showToastOnTabSwitch() {
		if (Controller
			...
			// Toast.makeText(this,"测试啊啊�?+ text, Toast.LENGTH_SHORT).show();
		}
	}
			...
			case CONTEXT_MENU_SHARE:
				if (mCurrentWebView != null
					...
					ApplicationUtils.sharePage(HomeActivity.this,
							"传�?正能量的321浏览器火爆登�?", " http://www.hi321.cn/");
				} else {
					if (b != null) {
						...
						case 1:
							...
							if (HomeActivity.INSTANCE.mCurrentWebView != null
									&& HomeActivity.INSTANCE.mCurrentWebView
											.getUrl().equals(
													Constants.URL_ABOUT_START)) {							ApplicationUtils.sharePage(HomeActivity.this,
										"传�?正能量的321浏览器火爆登�?",
										" http://www.hi321.cn/");
		...
		returnHomeTabBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (mUrlEditText != null
					...
					Toast.makeText(HomeActivity.this, "已经是主�?", 0).show();
					navigateToHome();
					return;
				} else {
					navigateToHome();
	...
	/**
	 * 设置PreviousBtn 按钮的背�?canGoBack true 表示还可以继续点击前�?	 * */
	...
	/**
	 * 设置NextBtn 按钮的背�?canNextBtn true 表示还可以继续点�?	 * */
	...
	public Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case PopMenu.Update:
				...
				// 如果想程序启动时自动�?��是否�?��更新�?把下面两行代码加在Activity 的onCreate()函数里�?
				com.umeng.common.Log.LOG = true;
				UmengUpdateAgent.setUpdateOnlyWifi(false); 			UmengUpdateAgent.setUpdateAutoPopup(false);
			...
			case PopWindowAddNewPage.LoadHomelPage:
				...
				updateUI();
				break;
			case PopWindowAddNewPage.SELECTPage:			int pos = (Integer) msg.obj;
				selectePosition = pos;
	 ...
	 * the clipboard. 选择文本并自动发送webview选中的文本到剪贴�?	 */
	...
	private void openAddBookmarkDialog() {
		if (HomeActivity.INSTANCE.mCurrentWebView != null
				&& !HomeActivity.INSTANCE.mCurrentWebView.getUrl().equals(
						Constants.URL_ABOUT_START)) {	Intent i = new Intent(this, EditBookmarkActivity.class);
	...
	UmengUpdateListener updateListener = new UmengUpdateListener() {
		...
		public void onUpdateReturned(int updateStatus, UpdateResponse updateInfo) {
			...
			case 1: // has no update
				...
				break;
			case 2: // none wifi
				Toast.makeText(HomeActivity.this, "没有wifi连接�?只在wifi下更�?",
						Toast.LENGTH_SHORT).show();
				break;
			case 3: // time out
