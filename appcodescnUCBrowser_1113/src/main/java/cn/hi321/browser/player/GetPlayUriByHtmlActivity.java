	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {			switch (msg.what) {
			case 0:
				...
				progressDialog.setVisibility(View.VISIBLE);// 显示进度对话�?				break;
			case 1:
				LogUtil.i(TAG, "View.GONE");
				if(progressDialog !=null)
				progressDialog.setVisibility(View.GONE);// 隐藏进度对话框，不可使用dismiss()、cancel(),否则再次调用show()时，显示的对话框小圆圈不会动�?				break;
			case 2:
				titleLeftButton();
				break;
			}
		...
		mWebview.setWebViewClient(new WebViewClient() {
			public boolean shouldOverrideUrlLoading(final WebView view,
				...
				if(Utils.isCheckNetAvailable(GetPlayUriByHtmlActivity.this)){
					...
					Toast.makeText(GetPlayUriByHtmlActivity.this, "网络不可用，请检查网络再�?", 0).show();
