...
public class DownloadTask extends AsyncTask<Void, Integer, Boolean> {
	...
	private int ev = -1; // ev : 0-完成下载�?1-�?��下载�?-系统异常
	...
	protected Boolean doInBackground(Void... params) {
		try {
			...
			mExceptionType = 1;		e.printStackTrace();
			return false;
		} catch (IOException e) {
	...
	private Boolean downloadToFile(DownloadJob job) throws  IOException {
		...
		if (job.getDownloadedSize() < job.getmTotalSize()) { 			mExceptionType = 5;// 网络异常
			return false;
		}
		return true;
	...
	private Boolean downloadFileByAndroidGet(DownloadJob job)
		...
		randomFile.close();
		in.close();
		if (job.getDownloadedSize() < job.getmTotalSize()) { // 解决有时没下载完就返回true，显示下载完�?			mExceptionType = 5;// 网络异常
			return false;
		}
		return true;
		...
		new Thread() {
			...
			public void run() {
				...
				if (null != HomeActivity.INSTANCE && !StringUtil.isEmpty(mid)) {
					// 当影片下载完成时，如果当前播放历史中播过该影片则将该影片的可播放百分比置�?
