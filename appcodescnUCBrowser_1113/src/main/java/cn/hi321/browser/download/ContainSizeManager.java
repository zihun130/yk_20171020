...
public class ContainSizeManager {
	...
	// 总容量，其他程序容量，风行容量，剩余大小(精度�?�?单位：G)
	private String funshionStr, freeStr; // (对应上面的数据的字符串表�?
	...
	/*
	 * // SDCard总大�?单位：GB,2位精�? public void countTotalSize() {
	 * 
	 * double total = Utils.getTotalExternaMemory() / 1024; totalStr =
	 * df.format(total); }
	 */

	// SDCard剩余空间大小（单位：GB�?位精度）
	...
	// SDCard\321Browser程序�?��的空间大小（单位：GB�?位精度）
	...
	Runnable ansynUpdateUi = new Runnable() {
		...
		@Override
		public void run() {
			// 找到视图并赋�?			sdcardView = (TextView) view.findViewById(R.id.sizeShow);
	 ...
	 * 
	 * @Override public void run() { try { // 取得相关�?count321BrowserSize();
	 * countFreeSize(); // 找到视图并赋�?// 总大�?sdcardView = (TextView)
