...
public class DownloadJobAdapter extends ArrayListAdapter<DownloadJob> {
	...
	private void downloadJobCompleted(ViewHolder holder, TouchListener tl,
		...
		if (DownloadHelper.getDownloadedFileSize(entity) == 0) {
			holder.downloadLength.setText("已完�?" + " / " + "该文件可能被人为删除或移�?");
			holder.btnPlay.setVisibility(View.GONE);
			holder.btnDelete.setVisibility(View.VISIBLE);
			holder.btnDelete.setOnTouchListener(tl);
		} else {
			holder.downloadLength.setText("已完�?"
					+ " / "
	...
	private class DownloadControlListener implements OnClickListener {
		...
		private void start() {
			...
			/*
			 * }else{ holder.progressText.setText("您的手机可能不支持断点下�?); }
			 */
		}
	}
	...
	class TouchListener implements OnTouchListener {
		...
		static class ViewHolder {
			TextView downloadName; 	TextView downloadLength;
			TextView progressText;
			ProgressBar progressBar;	RelativeLayout rlDownLayout;	Button downloadControl;
			Button btnDelete;
			Button btnPlay;
		}
